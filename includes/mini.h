...
/*   Updated: 2025/06/12 07:21:03 by aeleimat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool			initialize_token_copy(t_shell *shell, t_input *head, t_input **new_tokens, t_input **last_new);
int				process_token(t_tokenizer_state *state);
int				handle_quotes(t_tokenizer_state *state);
void			fush_token_buffer(t_tokenizer_state *state);
int				malloc_error1(void);
int				create_quoted_node(t_tokenizer_state *state, char *quoted_buf, bool is_double_quote);
char			*get_input(void);
void			cleanup_shell(t_shell *shell);
